...
def test_model_pipeline_creation():
    ...
    numeric_columns = ['age', 'capital_gain',
                       'capital_loss', 'fnlgt', 'hours_per_week']
    ...
    model_pipe = create_pipeline(
        rf_model, categorical_columns, numeric_columns, drop_columns)

    # check classifer
    assert isinstance(
        model_pipe[1], RandomForestClassifier), "Model is not Random Forest Classifier"
    ...
    assert isinstance(model_pipe[0].transformers[1][1],
                      StandardScaler), "Numeric encoder is not StandardScaler"
    assert isinstance(model_pipe[0].transformers[2][1][1],
                      OrdinalEncoder), "Categorical encoder is not OrdinalEncoder"
...
def assert_dataset_requirements():
    ...
    assert set(required_columns.keys()).issubset(
        df.columns), "Missing required columns in the dataset"
